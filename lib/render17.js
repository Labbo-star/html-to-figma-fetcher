...
async function renderPage(rawUrl, width, options = {}) {
  ...
  try {
    ...
    const siteChallenge = await page.evaluate(detectSiteChallenge);
    if (siteChallenge.blocked) {
      const error = new Error(`Сайт показывает проверку ${siteChallenge.provider}. Серверный импорт не получил содержимое страницы. Импортируйте HTML-файл или откройте доступ к сайту для сервера.`);
