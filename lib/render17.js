...
const { stabilizeTildaEntrances } = require('./tilda-entrance');
const { detectSiteChallenge } = require('./site-challenge');
...
async function assertNoSiteChallenge(page) {
  const challenge = await page.evaluate(detectSiteChallenge);
  if (!challenge.blocked) return;
  const error = new Error(`Сайт показывает проверку ${challenge.provider}. Серверный импорт не получил содержимое страницы. Импортируйте HTML-файл или откройте доступ к сайту для сервера.`);
  error.code = 'SITE_CHALLENGE';
  throw error;
}
...
async function renderPage(rawUrl, width, options = {}) {
  ...
  try {
    ...
    await assertNoSiteChallenge(page);
    ...
    // Tilda Zero Block reveals keep offscreen cards in an opacity:0 SBS
    // wrapper even after the lazy-load sweep. A static design needs the
    // completed entrance state for every card, including those below the fold.
    // Restrict this to identity-ending intoview effects: scroll timelines and
    // sliders express other states and must retain their own geometry.
    const tildaReveals = await page.evaluate(() => {
      const stats = { eligible: 0, stabilized: 0 };
      for (const el of document.querySelectorAll('[data-animate-sbs-event="intoview"][data-animate-sbs-opts]')) {
        const wrapper = el.querySelector(':scope > .tn-atom__sbs-anim-wrapper');
        if (!wrapper || el.closest('[data-html2figma-hide="1"]')) continue;
        let frames;
        try { frames = JSON.parse(el.getAttribute('data-animate-sbs-opts').replace(/'/g, '"')); } catch { continue; }
        if (!Array.isArray(frames) || !frames.length) continue;
        const last = frames[frames.length - 1];
        if (!last || typeof last !== 'object') continue;
        const neutral = (key, expected) => Number(last[key]) === expected;
        if (!neutral('op', 1) || !neutral('mx', 0) || !neutral('my', 0) ||
            !neutral('sx', 1) || !neutral('sy', 1) || !neutral('ro', 0)) continue;
        stats.eligible++;
        if (Number(getComputedStyle(wrapper).opacity) < .99) stats.stabilized++;
        wrapper.style.setProperty('opacity', '1', 'important');
        wrapper.style.setProperty('transform', 'none', 'important');
      }
      return stats;
    }).catch(() => ({ eligible: 0, stabilized: 0 }));

    stage = 'стабилизация входных анимаций Tilda';
    const tildaEntrances = await page.evaluate(stabilizeTildaEntrances).catch(() => ({ eligible: 0, revealed: 0, skipped: 0 }));

    stage = 'проверка доступа после загрузки';
    await assertNoSiteChallenge(page);
    ...
    const snapshot = await page.evaluate(({ maxLayers, maxHeight, viewportWidth }) => {
      ...
      let seq = 0, truncated = false, containerSeq = 0, imageCaptureSeq = 0, bgCaptureSeq = 0, textCaptureSeq = 0;
      ...
      const backgroundUrls = (e, backgroundImage) => {
        const rendered = urls(backgroundImage);
        if (!rendered.length || !e.classList || !e.classList.contains('t-bgimg')) return rendered;
        // Tilda may replace a full JPG with a computed /center/center/*.jpg.webp
        // optimization URL. That transient URL can 404 outside the site, even
        // when the browser paints the photo. Keep the authored full image URL.
        const authored = e.getAttribute('data-original') || e.getAttribute('data-bg') || e.getAttribute('data-background-image');
        if (!authored) return rendered;
        const original = fullTilda(authored);
        return /^https?:\/\//i.test(original) ? [original] : rendered;
      };
      ...
      const visible = (e, r, s) => {
        ...
        for (let i = 0; n && n !== doc.documentElement && i < 35; i++, n = n.parentElement) {
          // aria-hidden on the SVG itself is an accessibility annotation:
          // Elementor and icon packs use it on icons that remain painted.
          // An aria-hidden ancestor still excludes an inactive slide/dialog.
          if (n.hidden || (n.getAttribute('aria-hidden') === 'true' && !(n === e && e instanceof SVGElement))) return false;
      ...
      sectionEls.forEach((e, i) => {
        ...
        const sbg = backgroundUrls(e, s.backgroundImage), cap = sbg.length ? bgCaptureId(e) : undefined, sz = String(s.backgroundSize || 'cover'), pos = String(s.backgroundPosition || '50% 50%'), rep = String(s.backgroundRepeat || 'repeat');
      ...
      for (const e of candidates) {
        ...
        const bgUrls = backgroundUrls(e, s.backgroundImage), bgSize = String(s.backgroundSize || 'cover'), bgPos = String(s.backgroundPosition || '50% 50%'), bgRepeat = String(s.backgroundRepeat || 'repeat'), bgCap = bgUrls.length ? bgCaptureId(e) : undefined;
    ...
    snapshot.tildaReveals = tildaReveals;
    snapshot.tildaEntrances = tildaEntrances;
    // A static design cannot load a site-only webfont into Figma. Preserve
    // the browser-painted headline for substitution while leaving its editable
    // text layer in the snapshot. This is bounded and Tilda-specific.
    if (await page.evaluate(() => !!document.querySelector('#allrecords .t-rec'))) {
      const { captureTildaHeadlines } = require('./tilda-text-fidelity');
      snapshot.tildaTextFidelity = await captureTildaHeadlines(page, snapshot);
    }
    const captures = [];
    ...
    if (options.fidelity === true) {
      try {
        snapshot._fidelitySupplement = await collectBrowserFidelityFromPage(page);
      } catch (error) {
        snapshot._fidelityError = error && error.message ? String(error.message).slice(0, 160) : 'Не удалось уточнить слои';
      }
    }
    return { finalUrl: page.url(), snapshot, captures, referenceBuffer, qaReference };
  } catch (e) {
    const error = new Error(`${stage}: ${e && e.message ? e.message : e}`);
    error.code = e && e.code;
    throw error;
...
module.exports = async function handler(req, res) {
  ...
  try {
    ...
    const result = await renderPage(String(raw), width, { reference, qaPreview, fidelity: !reference });
    ...
    return res.status(e && e.code === 'SITE_CHALLENGE' ? 422 : 500).json({ ok: false, ...(e && e.code ? { code: e.code } : {}), error: e && e.message ? e.message : String(e) });
  }
};
