...
function sectionForY(snapshot, y, deepest = false) {
  const sections = Array.isArray(snapshot && snapshot.sections) ? snapshot.sections : [];
  const hits = sections.filter(s => y >= Number(s.y || 0) - 2 && y <= Number(s.y || 0) + Number(s.height || 0) + 2);
  // Keep existing ownership for backgrounds/widgets. An icon without a
  // captured parent belongs in the innermost section, above its background.
  const hit = deepest ? hits.sort((a, b) => Number(a.height || 0) - Number(b.height || 0))[0] : hits[0];
  ...
  for (const item of items) {
    ...
    try {
      ...
      snapshot.layers.push({
        ...
        sectionId: parent ? parent.sectionId : sectionForY(snapshot, y + 1, true),
...
async function buttonIconCaptures(page, snapshot) {
  const items = await page.evaluate(() => {
    ...
    for (const el of document.querySelectorAll('.elementor .elementor-button-icon svg,.elementor .elementor-button-icon i')) {
      // Font icons mark the SVG itself aria-hidden for screen readers. Only
      // skip an icon when an ancestor (such as a closed dialog) is hidden.
      if (el.parentElement?.closest('[aria-hidden="true"],dialog,[role="dialog"]')) continue;
      const r = el.getBoundingClientRect(), s = getComputedStyle(el);
