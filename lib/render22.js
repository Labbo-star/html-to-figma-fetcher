...
function frameworkOf(snapshot) {
  if (snapshot && snapshot.elementorPreflight && snapshot.elementorPreflight.detected) return 'elementor';
  ...
  for (const layer of layers.slice(0, 1200)) {
    ...
    if (/\.elementor(?:[.#\s-]|$)|\.e-con(?=[.#\s]|$)|\.e-flex(?=[.#\s]|$)|\[data-elementor-/i.test(probe)) elementor++;
  ...
  try {
    ...
    return res.status(200).json({
      ...
      stats: {
        ...
        tildaEntrances: snapshot.tildaEntrances || null,
...
module.exports._test = { frameworkOf };
