...
async function renderPage(rawUrl, width, options = {}) {
  ...
  try {
    ...
    const prepare = async () => page.evaluate(() => {
      ...
      const clipRectFor = root => {
        ...
        for (let i = 0; n && i < 10; i++, n = n.parentElement) {
          ...
          if (['hidden', 'clip'].includes(ox)) { result.left = Math.max(result.left, r.left); result.right = Math.min(result.right, r.right); }
          if (['hidden', 'clip'].includes(oy)) { result.top = Math.max(result.top, r.top); result.bottom = Math.min(result.bottom, r.bottom); }
        }
        return result;
      };
      const isVis = (el, clip) => {
      ...
      const lockItems = (root, items, active) => {
        ...
        if (!keep.length) {
          ...
          // The actual pixels on screen are authoritative. This preserves two-card
          // carousels while overlapping hero slides collapse to one initial slide.
    ...
    const snapshot = await page.evaluate(({ maxLayers, maxHeight, viewportWidth }) => {
      ...
      return { width: viewportWidth, height, sections, layers, truncated, rendererVersion: 16 };
...
module.exports = async function handler(req, res) {
  ...
  if (req.method === 'GET' && String(req.query.ping || '') === '1') return res.status(200).json({ ok: true, service: 'browser-renderer', version: 16, visualQa: true, clippedText: true, formText: true, stableInitialSliders: true, stackPaths: true });
  let body = req.body;
  ...
  try {
    ...
    const result = await renderPage(String(raw), width, { reference });
    ...
    return res.status(200).json({ ok: true, mode: 'browser-snapshot-v16', finalUrl: result.finalUrl, snapshot: result.snapshot, stats: { layers: result.snapshot.layers.length, sections: result.snapshot.sections.length, truncated: !!result.snapshot.truncated, imageLayers: result.snapshot.layers.filter(x => x.kind === 'image').length } });
  } catch (e) {
