  ...
  try {
    ...
    const prepare = async () => page.evaluate(() => {
      ...
      for (const arr of groups.values()) {
        ...
        arr.sort((a, b) => Number(a.getAttribute('data-slide-index') || 0) - Number(b.getAttribute('data-slide-index') || 0));
        arr.forEach((el, i) => i === 0 ? show(el) : hide(el));
      }
    ...
    const snapshot = await page.evaluate(({ maxLayers, maxHeight, viewportWidth }) => {
      ...
      for (const [owner, nodes] of textGroups.entries()) {
        ...
        const sectionId = sectionFor(owner, r), ownKey = semantic.get(owner), parentKey = ownKey || nearest(owner), abs = { x: round(v.x), y: round(v.y), width: round(v.width), height: round(v.height) }, base = relative(abs, parentKey), fs = num(s.fontSize, 16), lh = s.lineHeight === 'normal' ? (v.lineCount > 1 ? Math.max(fs, round((v.height + fs * .25) / v.lineCount)) : fs * 1.2) : num(s.lineHeight, fs * 1.2), ta = String(s.textAlign || 'left').toUpperCase();
